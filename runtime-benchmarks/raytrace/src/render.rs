...
fn color(rng: &mut XorShiftRng, mut r: Ray, model: &Model) -> Vec3 {
    ...
    while let Some(hit) = model.hit(&r) {
        let scattered = hit.material.scatter(rng, &r, &hit);
...
pub fn render(rng: &mut XorShiftRng,
              scene: &Model,
              camera: &Camera,
              width: usize,
              height: usize,
